...
Base=np.zeros((2**N,2**N),dtype=np.complex128)
Base=Base+J*(NKron(PX,PX,id(2**(N-2))) + NKron(id(2**(N-2)),PX,PX))
Base=Base+J*(NKron(PY,PY,id(2**(N-2))) + NKron(id(2**(N-2)),PY,PY))
Base=Base+U*(NKron(PZ,PZ,id(2**(N-2))) + NKron(id(2**(N-2)),PZ,PZ))
Base=Base+Xi*(NKron(PX,id(2**(N-1)))+NKron(id(2**(N-1)),PX))

i=1
while i<N-1:
    Base=Base+J*(NKron(id(2**i),PX,PX,id(2**(N-2-i))))
    Base=Base+J*(NKron(id(2**i),PY,PY,id(2**(N-2-i))))
    Base=Base+U*(NKron(id(2**i),PZ,PZ,id(2**(N-2-i))))
    Base=Base+Xi*(NKron(id(2**i),PX,id(2**(N-i-1))))
    i+=1
